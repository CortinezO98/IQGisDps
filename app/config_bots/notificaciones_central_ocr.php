...
// ini_set('display_errors', '1');
    ...
    $consulta_notificaciones = mysqli_query($enlace_db, "SELECT `nc_id`, `nc_id_modulo`, `nc_prioridad`, `nc_id_set_from`, `nc_address`, `nc_cc`, `nc_bcc`, `nc_reply_to`, `nc_subject`, `nc_body`, `nc_embeddedimage_ruta`, `nc_intentos`, `nc_eliminar`, `nc_estado_envio`, `ncr_host`, `ncr_port`, `ncr_smtpsecure`, `ncr_smtpauth`, `ncr_username`, `ncr_password`, `ncr_setfrom`, `ncr_setfrom_name`, `nc_embeddedimage_ruta`, `nc_embeddedimage_nombre`, `nc_embeddedimage_tipo` FROM `administrador_notificaciones` LEFT JOIN `administrador_buzones` AS RT ON `administrador_notificaciones`.`nc_id_set_from`=RT.`ncr_id` WHERE `nc_estado_envio`='Pendiente' AND `nc_id_set_from`='2' ORDER BY `nc_prioridad` LIMIT 8 OFFSET 0");
        ...
        for ($i=0; $i < count($resultado_notificaciones); $i++) { 
            ...
            if ($resultado_notificaciones[$i][14]!="" AND $resultado_notificaciones[$i][15]!="" AND $resultado_notificaciones[$i][16]!="" AND $resultado_notificaciones[$i][17]!="" AND $resultado_notificaciones[$i][18]!="" AND $resultado_notificaciones[$i][19]!="" AND $resultado_notificaciones[$i][20]!="" AND $resultado_notificaciones[$i][21]!="" AND $resultado_notificaciones[$i][4]!="" AND $resultado_notificaciones[$i][8]!="" AND $resultado_notificaciones[$i][9]!="") {
                try {
                    ...
                    $mail->AddAttachment(UPLOADS_ROOT.'Manual_de_ciudadano_para_creacion_y_envio_de_documentos_solicitados_en_archivo_PDF.pdf', 'Manual_de_ciudadano_para_creacion_y_envio_de_documentos_solicitados_en_archivo_PDF.pdf');
                    $mail->AddAttachment(UPLOADS_ROOT.'Contrato_Social.pdf', 'Contrato_Social.pdf');
