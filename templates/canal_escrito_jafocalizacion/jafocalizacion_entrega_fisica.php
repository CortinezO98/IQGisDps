<?php
  ...
  $subtitle = "Jóvenes en Acción y Focalización | 8. Formato Entrega Física | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cejef_radicado_salida` LIKE ? OR `cejef_radicado_entrada` LIKE ? OR `cejef_destinatario` LIKE ? OR `cejef_direccion` LIKE ? OR `cejef_departamento` LIKE ? OR `cejef_municipio` LIKE ? OR `cejef_observaciones` LIKE ? OR `cejef_notificar` LIKE ? OR `cejef_registro_usuario` LIKE ? OR `cejef_registro_fecha` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ? OR TC.`ciu_departamento` LIKE ? OR TC.`ciu_municipio`)";
  ...
  if ($permisos_usuario=="Administrador") {
      ...
      $filtro_perfil=" AND `cejef_registro_usuario`=?";
      array_push($data_consulta, $_SESSION[APP_SESSION.'_session_usu_id']);
  }

  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cejef_registro_fecha`>=?)";
  ...
  $consulta_contar_string="SELECT COUNT(`cejef_id`) FROM `gestion_cejafo_entrega_fisica` LEFT JOIN `administrador_ciudades` AS TC ON `gestion_cejafo_entrega_fisica`.`cejef_municipio`=TC.`ciu_codigo` LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_entrega_fisica`.`cejef_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="jafocalizacion_entrega_fisica_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="jafocalizacion_entrega_fisica?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="jafocalizacion_entrega_fisica?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
        ...
        <!-- modal reportes -->
        <?php require_once('jafocalizacion_entrega_fisica_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
