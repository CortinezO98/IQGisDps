<?php
  ...
  $subtitle = "Jóvenes en Acción y Focalización | 5. Formato Gestión de Novedades JeA | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cejgn_id_novedad` LIKE ? OR `cejgn_id_persona` LIKE ? OR `cejgn_fecha_gestion` LIKE ? OR `cejgn_estado` LIKE ? OR `cejgn_tipo_rechazo` LIKE ? OR `cejgn_observacion_rechazo` LIKE ? OR `cejgn_correccion_datos_sija` LIKE ? OR `cejgn_codigo_novedad` LIKE ? OR `cejgn_observaciones` LIKE ? OR `cejgn_notificar` LIKE ? OR `cejgn_registro_usuario` LIKE ? OR `cejgn_registro_fecha` LIKE ? OR ESTADO.`ceco_valor` LIKE ? OR TIPORECHAZO.`ceco_valor` LIKE ? OR DATOSSIJA.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cejgn_registro_fecha`>=?)";
  ...
  $consulta_contar_string="SELECT COUNT(`cejgn_id`) FROM `gestion_cejafo_gestion_novedades` LEFT JOIN `gestion_ce_configuracion` AS ESTADO ON `gestion_cejafo_gestion_novedades`.`cejgn_estado`=ESTADO.`ceco_id`
   ...
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_gestion_novedades`.`cejgn_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  $consulta_string="SELECT `cejgn_id`, `cejgn_id_novedad`, `cejgn_id_persona`, `cejgn_fecha_gestion`, `cejgn_estado`, `cejgn_tipo_rechazo`, `cejgn_observacion_rechazo`, `cejgn_correccion_datos_sija`, `cejgn_codigo_novedad`, `cejgn_observaciones`, `cejgn_notificar`, `cejgn_registro_usuario`, `cejgn_registro_fecha`, ESTADO.`ceco_valor`, TIPORECHAZO.`ceco_valor`, DATOSSIJA.`ceco_valor`, TU.`usu_nombres_apellidos` FROM `gestion_cejafo_gestion_novedades` 
   ...
   LEFT JOIN `gestion_ce_configuracion` AS TIPORECHAZO ON `gestion_cejafo_gestion_novedades`.`cejgn_tipo_rechazo`=TIPORECHAZO.`ceco_id`
   LEFT JOIN `gestion_ce_configuracion` AS DATOSSIJA ON `gestion_cejafo_gestion_novedades`.`cejgn_correccion_datos_sija`=DATOSSIJA.`ceco_id`
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_gestion_novedades`.`cejgn_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `cejgn_id` DESC LIMIT ?,?";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="jafocalizacion_gestion_novedades_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="jafocalizacion_gestion_novedades?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="jafocalizacion_gestion_novedades?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Id Novedad</th>
                          <th class="px-1 py-2">Id Persona</th>
                          <th class="px-1 py-2">Fecha Gestión</th>
                          <th class="px-1 py-2">Estado</th>
                          <th class="px-1 py-2">Tipo Rechazo</th>
                          <th class="px-1 py-2">Observación de Rechazo</th>
                          <th class="px-1 py-2">Corrección Datos SIJA</th>
                          <th class="px-1 py-2">Código de la Novedad</th>
        ...
        <!-- modal reportes -->
        <?php require_once('jafocalizacion_gestion_novedades_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
