<?php
  ...
  $subtitle = "Jóvenes en Acción y Focalización | 1. Proyección de Peticiones Vivienda | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cejpp_radicado_entrada` LIKE ? OR `cejpp_proyector` LIKE ? OR `cejpp_novedad_radicado` LIKE ? OR `cejpp_formato` LIKE ? OR `cejpp_identificacion_peticionario` LIKE ? OR `cejpp_nombre_peticionario` LIKE ? OR `cejpp_correo` LIKE ? OR `cejpp_observaciones` LIKE ? OR `cejpp_notificar` LIKE ? OR `cejpp_registro_usuario` LIKE ? OR `cejpp_registro_fecha` LIKE ? OR NOVEDAD.`ceco_valor` LIKE ? OR FORMATO.`ceco_valor` LIKE ? OR PROYECTOR.`usu_nombres_apellidos` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cejpp_registro_fecha`>=?)";
  ...
  $consulta_contar_string="SELECT COUNT(`cejpp_id`) FROM `gestion_cejafo_proyeccion_peticiones` LEFT JOIN `gestion_ce_configuracion` AS NOVEDAD ON `gestion_cejafo_proyeccion_peticiones`.`cejpp_novedad_radicado`=NOVEDAD.`ceco_id`
   ...
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_proyeccion_peticiones`.`cejpp_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  $consulta_string="SELECT `cejpp_id`, `cejpp_radicado_entrada`, `cejpp_proyector`, `cejpp_novedad_radicado`, `cejpp_formato`, `cejpp_identificacion_peticionario`, `cejpp_nombre_peticionario`, `cejpp_correo`, `cejpp_observaciones`, `cejpp_notificar`, `cejpp_registro_usuario`, `cejpp_registro_fecha`, NOVEDAD.`ceco_valor`, FORMATO.`ceco_valor`, PROYECTOR.`usu_nombres_apellidos`, TU.`usu_nombres_apellidos` FROM `gestion_cejafo_proyeccion_peticiones` LEFT JOIN `gestion_ce_configuracion` AS NOVEDAD ON `gestion_cejafo_proyeccion_peticiones`.`cejpp_novedad_radicado`=NOVEDAD.`ceco_id`
   LEFT JOIN `gestion_ce_configuracion` AS FORMATO ON `gestion_cejafo_proyeccion_peticiones`.`cejpp_formato`=FORMATO.`ceco_id`
   LEFT JOIN `administrador_usuario` AS PROYECTOR ON `gestion_cejafo_proyeccion_peticiones`.`cejpp_proyector`=PROYECTOR.`usu_id`
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_proyeccion_peticiones`.`cejpp_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `cejpp_id` DESC LIMIT ?,?";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="jafocalizacion_proyeccion_peticiones_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="jafocalizacion_proyeccion_peticiones?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="jafocalizacion_proyeccion_peticiones?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Formato</th>
                          <th class="px-1 py-2">Número Identificación Peticionario</th>
        ...
        <!-- modal reportes -->
        <?php require_once('jafocalizacion_proyeccion_peticiones_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
