<?php
    ...
    if(isset($_POST["reporte"])){
        ...
        $titulo_reporte="Canal Escrito - JAFocalización - Formato de Relación RAE JeA - ".date('Y_m_d H_i_s').".csv";
        ...
        $consulta_string="SELECT `cejrr_id`, `cejrr_radicado_salida`, `cejrr_radicado_entrada`, `cejrr_destinatario`, `cejrr_direccion`, `cejrr_municipio`, `cejrr_modalidad_envio`, `cejrr_srjv`, `cejrr_proyector`, `cejrr_aprobador`, `cejrr_firma`, `cejrr_cedula_firmante`, `cejrr_fecha_gestion_rae`, `cejrr_fecha_envio`, `cejrr_qq`, `cejrr_observaciones`, `cejrr_notificar`, `cejrr_registro_usuario`, `cejrr_registro_fecha`, MODALIDADENVIO.`ceco_valor`, SRJV.`ceco_valor`, FIRMA.`ceco_valor`, TU.`usu_nombres_apellidos`, TCIU.`ciu_departamento`, TCIU.`ciu_municipio`, PROYECTOR.`usu_nombres_apellidos`, APROBADOR.`usu_nombres_apellidos` FROM `gestion_cejafo_relacion_rae` LEFT JOIN `gestion_ce_configuracion` AS MODALIDADENVIO ON `gestion_cejafo_relacion_rae`.`cejrr_modalidad_envio`=MODALIDADENVIO.`ceco_id`
             ...
             LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_relacion_rae`.`cejrr_registro_usuario`=TU.`usu_id` WHERE `cejrr_registro_fecha`>=? AND `cejrr_registro_fecha`<=? ".$filtro_perfil." ORDER BY `cejrr_id`";
    ...
    fputcsv($file, array('Reporte: Canal Escrito JAFocalización - 3. Formato de Relación RAE JeA'), $delimitador, $encapsulador);
    fputcsv($file, array('Fecha filtro: '.$fecha_inicio.' A '.$fecha_fin), $delimitador, $encapsulador);
    
    $titulos=array('Radicado Salida', 
'Radicado Entrada', 
'Destinatario', 
'Dirección', 
'Municipio', 
...
'Doc Usuario Registro', 'Registrado por', 'Fecha Registro');
    ...
    for ($i=0; $i < count($resultado_registros); $i++) {
        $linea=array($resultado_registros[$i][1], $resultado_registros[$i][2], $resultado_registros[$i][3], $resultado_registros[$i][4], $resultado_registros[$i][24], $resultado_registros[$i][23], $resultado_registros[$i][19], $resultado_registros[$i][20], $resultado_registros[$i][25], $resultado_registros[$i][26], $resultado_registros[$i][21], $resultado_registros[$i][11], $resultado_registros[$i][12], $resultado_registros[$i][13], $resultado_registros[$i][14], $resultado_registros[$i][15], $resultado_registros[$i][17], $resultado_registros[$i][22], $resultado_registros[$i][18]);
