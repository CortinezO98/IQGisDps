    ...
    for ($k=0; $k < count($array_dias_mes); $k++) { 
            ...
            //1. Proyección Consolidación
              ...
              if (count($resultado_registros)) {
                for ($i=0; $i < count($resultado_registros); $i++) { 
                  $array_datos_gestion[$id_formulario]['agentes'][]=$resultado_registros[$i][0];
                  $array_datos_gestion[$id_formulario]['agentes']=array_values(array_unique($array_datos_gestion[$id_formulario]['agentes']));
                  ...
                  if (!isset($array_datos_gestion[$id_formulario]['gestion_agente_hora'][$resultado_registros[$i][0]])) {
                    $array_datos_gestion[$id_formulario]['gestion_agente_hora'][$resultado_registros[$i][0]]=$array_anio_mes_hora_val;
                    // $array_datos_gestion[$id_formulario]['gestion_agente'][$resultado_registros[$i][0]]['fecha']=$array_dias_mes_data;
                  }

                  $array_datos_gestion[$id_formulario]['gestion_agente_fecha'][$resultado_registros[$i][0]][$resultado_registros[$i][1]]+=$resultado_registros[$i][3];
                  $array_datos_gestion[$id_formulario]['gestion_agente_fecha_lista'][$resultado_registros[$i][0]][]=$resultado_registros[$i][1];
                  $array_datos_gestion[$id_formulario]['gestion_agente_hora'][$resultado_registros[$i][0]][$resultado_registros[$i][2]]+=$resultado_registros[$i][3];
                  ...
                  $array_datos_gestion[$id_formulario]['gestion_agente_fecha_lista'][$resultado_registros[$i][0]]=array_values(array_unique($array_datos_gestion[$id_formulario]['gestion_agente_fecha_lista'][$resultado_registros[$i][0]]));
                }
                ...
                // if (date('Y-m-d', strtotime($_SESSION[APP_SESSION.'_session_ce_productividad']['fecha_inicio']))==date('Y-m-d', strtotime($_SESSION[APP_SESSION.'_session_ce_productividad']['fecha_fin']))) {
                  for ($i=0; $i < count($array_datos_gestion[$id_formulario]['agentes']); $i++) {
                    $id_agente_prod=$array_datos_gestion[$id_formulario]['agentes'][$i];
                    ...
                    for ($j=0; $j < count($array_datos_gestion[$id_formulario]['gestion_agente_fecha_lista'][$id_agente_prod]); $j++) { 
                      $fecha_agente=$array_datos_gestion[$id_formulario]['gestion_agente_fecha_lista'][$id_agente_prod][$j];
                      $productividad_agente_fecha=maximo100(($array_datos_gestion[$id_formulario]['gestion_agente_fecha'][$id_agente_prod][$fecha_agente]*100)/$array_metas[$id_formulario]['meta'][$fecha_agente]);
                      $productividad_agente_array[]=$productividad_agente_fecha;
                      // $meta_agente_array[]=$array_metas[$id_formulario]['meta'][$fecha_agente];
                      // $gestiones_agente_array[]=$array_datos_gestion[$id_formulario]['gestion_agente_fecha'][$id_agente_prod][$fecha_agente];
                      // $gestiones_hora_array[]=$array_datos_gestion[$id_formulario]['gestion_agente_fecha'][$id_agente_prod][$fecha_agente]/8;
                    ...
                    $array_resumen[$id_agente_prod]['productividad_total'][$fecha_resumen][]=$productividad_agente_total;

                    //INSERT-UPDATE HISTORIAL PRODUCTIVIDAD
                    $cep_id=$id_agente_prod.'-'.$id_formulario.'-'.$_SESSION[APP_SESSION.'_session_ce_productividad']['fecha_inicio'];
                    $cep_formulario=$id_formulario;
                    $cep_agente=$id_agente_prod;
                    $cep_coordinador=$array_datos_agente[$id_agente_prod]['coordinador'];
                    $cep_fecha=$_SESSION[APP_SESSION.'_session_ce_productividad']['fecha_inicio'];
                    $cep_meta=$array_metas[$id_formulario]['meta'][$fecha_agente];
                    $cep_gestiones=$array_datos_gestion[$id_formulario]['gestion_agente_fecha'][$id_agente_prod][$fecha_agente];
                    $cep_productividad=$productividad_agente_total;
        ...
        //JAFOCALIZACIÓN
            ...
            //4. Formato de Gestión de Correos
              ...
              $consulta_string="SELECT `cejgc_registro_usuario`, DATE_FORMAT(`cejgc_registro_fecha`, '%Y-%m-%d') AS FECHA, DATE_FORMAT(`cejgc_registro_fecha`, '%H') AS HORA, COUNT(`cejgc_registro_usuario`) AS CONTEO, TU.`usu_nombres_apellidos`, TC.`usu_nombres_apellidos`, TC.`usu_id` FROM `gestion_cejafo_gestion_correo` LEFT JOIN `administrador_usuario` AS TU ON `gestion_cejafo_gestion_correo`.`cejgc_registro_usuario`=TU.`usu_id` LEFT JOIN `administrador_usuario` AS TC ON TU.`usu_supervisor`=TC.`usu_id` WHERE 1=1 ".$filtro_fechas_ja_p4." GROUP BY `cejgc_registro_usuario`, FECHA, HORA";
              $consulta_registros = $enlace_db->prepare($consulta_string);
              if (count($data_consulta)>0) {
                  $consulta_registros->bind_param(str_repeat("s", count($data_consulta)), ...$data_consulta);
              }
        ...
        for ($i=0; $i < count($resultado_registros_justificacion); $i++) {
          if ($resultado_registros_justificacion[$i][8]!='') {
            ...
            $array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['gestion_agente'][$resultado_registros_justificacion[$i][2]]['novedad']=$resultado_registros_justificacion[$i][9];
          }

          if ($resultado_registros_justificacion[$i][13]=='') {
            $array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['gestion_agente'][$resultado_registros_justificacion[$i][2]][$resultado_registros_justificacion[$i][4]]['ajustada']=0;
            $array_resumen[$resultado_registros_justificacion[$i][2]]['productividad_total_ajustada'][]=0;
          } else {
            $array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['gestion_agente'][$resultado_registros_justificacion[$i][2]][$resultado_registros_justificacion[$i][4]]['ajustada']=$resultado_registros_justificacion[$i][13];
          ...
          $array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['promedio_suma']+=$productividad_suma;

          $array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['promedio_general']=$array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['promedio_suma']/count($array_datos_justificacion[$resultado_registros_justificacion[$i][1]]['gestion_agente']);
            
        }
    }


    // FALTA REVISAR TIPOLOGÍA
    // FALTA REVISAR PRODUCTIVIDAD AJUSTADA
    // REPLICAR A TODOS LOS FORMS
  }

  // echo "<pre>";
  // print_r($array_resumen);
  // echo "</pre>";
