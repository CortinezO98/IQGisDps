<?php
  ...
  $subtitle = "Reparto | 2. Aprobación Firma FA | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`ceaff_radicado` LIKE ? OR `ceaff_proyector` LIKE ? OR `ceaff_estado` LIKE ? OR `ceaff_observaciones` LIKE ? OR `ceaff_notificar` LIKE ? OR `ceaff_registro_usuario` LIKE ? OR `ceaff_registro_fecha` LIKE ? OR PROYECTOR.`usu_nombres_apellidos` LIKE ? OR ESTADO.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`ceaff_registro_fecha`>=?)";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_aprobacion_firma_fa_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_aprobacion_firma_fa?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_aprobacion_firma_fa?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_aprobacion_firma_fa_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
