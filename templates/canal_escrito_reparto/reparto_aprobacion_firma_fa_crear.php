<?php
  ...
  /*VARIABLES*/
  $title = "Canal Escrito";
  $subtitle = "Reparto | 2. Aprobación Firma FA | Crear Registro";
  $pagina=validar_input($_GET['pagina']);
  $filtro_permanente=validar_input($_GET['id']);
  $bandeja=validar_input(base64_decode($_GET['bandeja']));
  $parametros_add='';
  $url_salir="reparto_aprobacion_firma_fa?pagina=".$pagina."&id=".$filtro_permanente."&bandeja=".base64_encode($bandeja);
// error_reporting(E_ALL);
// ini_set('display_errors', '1');
  
  $consulta_string_parametros="SELECT `ceco_id`, `ceco_formulario`, `ceco_campo`, `ceco_valor`, `ceco_estado`, `ceco_actualiza_usuario`, `ceco_actualiza_fecha`, `ceco_registro_usuario`, `ceco_registro_fecha` FROM `gestion_ce_configuracion` WHERE `ceco_formulario`='aprobacion_firma_fa' AND `ceco_estado`='Activo' ORDER BY `ceco_campo`, `ceco_valor`";
  ...
  if(isset($_POST["guardar_registro"])){
      $ceaff_radicado=validar_input($_POST['ceaff_radicado']);
      $ceaff_proyector=validar_input($_POST['ceaff_proyector']);
      $ceaff_estado=validar_input($_POST['ceaff_estado']);
      $ceaff_observaciones=validar_input($_POST['ceaff_observaciones']);
      $ceaff_notificar=validar_input($_POST['notificar']);
      $ceaff_registro_usuario=$_SESSION[APP_SESSION.'_session_usu_id'];

      if($_SESSION[APP_SESSION.'_registro_creado_canal_escrito']!=1){
          // Prepara la sentencia
          $sentencia_insert = $enlace_db->prepare("INSERT INTO `gestion_cerep_aprobacion_firma_fa`(`ceaff_radicado`, `ceaff_proyector`, `ceaff_estado`, `ceaff_observaciones`, `ceaff_notificar`, `ceaff_registro_usuario`) VALUES (?,?,?,?,?,?)");
          // Agrega variables a sentencia preparada
          $sentencia_insert->bind_param('ssssss', $ceaff_radicado, $ceaff_proyector, $ceaff_estado, $ceaff_observaciones, $ceaff_notificar, $ceaff_registro_usuario);
          // Prepara la sentencia
          ...
          if ($sentencia_insert->execute()) {
              ...
              if ($ceaff_notificar=='Si') {
                //PROGRAMACIÓN NOTIFICACIÓN
                $asunto='2. Aprobación Firma FA - Reparto | Canal Escrito';
                $referencia='2. Aprobación Firma FA - Reparto | Canal Escrito';
                $contenido="<p style='font-size: 12px;padding: 0px 5px 0px 5px; color: #666666;'>Cordial saludo,<br><br>Se ha generado el siguiente registro:</p>
                  <table style='width: 500px; font-size: 13px; font-family: Lato, Arial, sans-serif;'>
                    <tr>
                      ...
                      <td style='width: 70%;padding: 5px 5px 5px 5px;background-color: #F2F2F2;'>".$ceaff_radicado."</td>
                    </tr>
                    <tr>
                      <td style='width: 30%;background-color: #1C2262; color: #FFFFFF; padding: 5px 5px 5px 5px; text-align: center;'>Proyector</td>
                      <td style='width: 70%;padding: 5px 5px 5px 5px;background-color: #F2F2F2;'>".$array_analista[$ceaff_proyector]."</td>
                    </tr>
                    <tr>
                      <td style='width: 30%;background-color: #1C2262; color: #FFFFFF; padding: 5px 5px 5px 5px; text-align: center;'>Estado</td>
                      <td style='width: 70%;padding: 5px 5px 5px 5px;background-color: #F2F2F2;'>".$array_parametros['estado']['texto'][$ceaff_estado]."</td>
                    </tr>
                    <tr>
                      <td style='width: 30%;background-color: #1C2262; color: #FFFFFF; padding: 5px 5px 5px 5px; text-align: center;'>Observaciones</td>
                      <td style='width: 70%;padding: 5px 5px 5px 5px;background-color: #F2F2F2;'>".$ceaff_observaciones."</td>
                    </tr>
                    ...
                    <div class="card-body row">
                      <div class="col-md-12">
                          <div class="form-group my-1">
                            <label for="ceaff_radicado" class="my-0">Radicado</label>
                            <input type="text" class="form-control form-control-sm" name="ceaff_radicado" id="ceaff_radicado" minlength="18" maxlength="18" value="<?php if(isset($_POST["guardar_registro"])){ echo $ceaff_radicado; } ?>" <?php if($_SESSION[APP_SESSION.'_registro_creado_canal_escrito']==1) { echo 'readonly'; } ?> autocomplete="off" required>
                          </div>
                      </div>
                      <div class="col-md-12">
                          <div class="form-group my-1">
                              <label for="ceaff_proyector" class="my-0">Proyector</label>
                              <select class="selectpicker form-control form-control-sm form-select" data-live-search="true" data-container="body" name="ceaff_proyector" id="ceaff_proyector" title="Seleccione" <?php if($_SESSION[APP_SESSION.'_registro_creado_canal_escrito']==1) { echo 'disabled'; } ?> required>
                                  <?php for ($i=0; $i < count($resultado_registros_analistas); $i++): ?>
                                    <option value="<?php echo $resultado_registros_analistas[$i][0]; ?>" <?php if($ceaff_proyector==$resultado_registros_analistas[$i][0]){ echo "selected"; } ?>><?php echo $resultado_registros_analistas[$i][1]; ?></option>
                                  <?php endfor; ?>
                              </select>
                          </div>
                      </div>
                      <div class="col-md-12">
                          <div class="form-group my-1">
                              <label for="ceaff_estado" class="my-0">Estado</label>
                              <select class="form-control form-control-sm form-select" name="ceaff_estado" id="ceaff_estado" <?php if($_SESSION[APP_SESSION.'_registro_creado_canal_escrito']==1) { echo 'disabled'; } ?> required>
                                  <option class="font-size-11" value="">Seleccione</option>
                                  <?php for ($i=0; $i < count($array_parametros['estado']['id']); $i++): ?>
                                    <option value="<?php echo $array_parametros['estado']['id'][$i]; ?>" <?php if($ceaff_estado==$array_parametros['estado']['id'][$i]){ echo "selected"; } ?>><?php echo $array_parametros['estado']['valor'][$i]; ?></option>
                                  <?php endfor; ?>
                              </select>
                          </div>
                      </div>
                      <div class="col-md-12">
                          <div class="form-group">
                            <label for="ceaff_observaciones" class="my-0">Observaciones</label>
                            <textarea class="form-control form-control-sm height-100" name="ceaff_observaciones" id="ceaff_observaciones" <?php if($_SESSION[APP_SESSION.'_registro_creado_canal_escrito']==1) { echo 'readonly'; } ?>><?php if(isset($_POST["guardar_registro"])){ echo $ceaff_observaciones; } ?></textarea>
                          </div>
                      </div>
                      <div class="col-md-12">
                          <div class="form-group">
