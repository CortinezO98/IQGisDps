<?php
  ...
  $subtitle = "Reparto | 3. Firma FA | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`ceff_radicado_entrada` LIKE ? OR `ceff_radicado_salida` LIKE ? OR `ceff_modalidad_envio` LIKE ? OR `ceff_observaciones` LIKE ? OR `ceff_notificar` LIKE ? OR `ceff_registro_usuario` LIKE ? OR `ceff_registro_fecha` LIKE ? OR MENVIO.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if ($permisos_usuario=="Administrador") {
      ...
      $filtro_perfil=" AND `ceff_registro_usuario`=?";
      array_push($data_consulta, $_SESSION[APP_SESSION.'_session_usu_id']);
  }

  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`ceff_registro_fecha`>=?)";
  ...
  $consulta_contar_string="SELECT COUNT(`ceff_id`) FROM `gestion_cerep_firma_fa` LEFT JOIN `gestion_ce_configuracion` AS MENVIO ON `gestion_cerep_firma_fa`.`ceff_modalidad_envio`=MENVIO.`ceco_id`  LEFT JOIN `administrador_usuario` AS TU ON `gestion_cerep_firma_fa`.`ceff_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_firma_fa_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_firma_fa?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_firma_fa?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_firma_fa_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
