<?php
  ...
  $subtitle = "Reparto | 7. Firma Traslados | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`ceft_radicado_entrada` LIKE ? OR `ceft_radicado_salida` LIKE ? OR `ceft_rechazos` LIKE ? OR `ceft_forma` LIKE ? OR `ceft_proyector` LIKE ? OR `ceft_inspector` LIKE ? OR `ceft_aprobador` LIKE ? OR `ceft_observaciones` LIKE ? OR `ceft_notificar` LIKE ? OR `ceft_registro_usuario` LIKE ? OR `ceft_registro_fecha` LIKE ? OR RECHAZOS.`ceco_valor` LIKE ? OR FORMA.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`ceft_registro_fecha`>=?)";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_firma_traslados_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_firma_traslados?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_firma_traslados?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Radicado Entrada o ERP</th>
                          <th class="px-1 py-2">Radicado Salida</th>
                          <th class="px-1 py-2">Rechazos</th>
                          <th class="px-1 py-2">Forma</th>
                          <th class="px-1 py-2">Proyector</th>
                          <th class="px-1 py-2">Inspector</th>
                          ...
                          <td class="p-1 font-size-11">
                            <?php
                              ...
                              for ($j=0; $j < count($ceft_forma); $j++) { 
                                  if ($ceft_forma[$j]!="") {
                                    $ceft_forma_mostrar.=$array_parametros['forma']['texto'][$ceft_forma[$j]].'<br>';
                                  }
                              }
                            ?>
                            <?php echo $ceft_forma_mostrar; ?>
                          </td>
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_firma_traslados_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
