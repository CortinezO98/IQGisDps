<?php
    ...
    if(isset($_POST["reporte"])){
        ...
        $titulo_reporte="Canal Escrito - Proyección FA - ".date('Y_m_d H_i_s').".csv";
    ...
    fputcsv($file, array('Reporte: Canal Escrito - Proyección FA'), $delimitador, $encapsulador);
    fputcsv($file, array('Fecha filtro: '.$fecha_inicio.' A '.$fecha_fin), $delimitador, $encapsulador);
    
    $titulos=array('Radicado Entrada', 'Documento Identidad', 'Nombre Ciudadano', 'Correo/Dirección Notificación', 'Departamento', 'Solicitud/Novedad Radicado', 'Observaciones', 'Doc Usuario Registro', 'Registrado por', 'Fecha Registro');

    fputcsv($file, $titulos, $delimitador, $encapsulador);

    for ($i=0; $i < count($resultado_registros); $i++) {
        $linea=array($resultado_registros[$i][1], $resultado_registros[$i][2], $resultado_registros[$i][3], $resultado_registros[$i][4], $resultado_registros[$i][11], $resultado_registros[$i][12], $resultado_registros[$i][7], $resultado_registros[$i][9], $resultado_registros[$i][13], $resultado_registros[$i][10]);
