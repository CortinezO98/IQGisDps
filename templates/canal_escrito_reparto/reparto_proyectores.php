<?php
  ...
  $subtitle = "Reparto | 8. Proyectores | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cep_radicado_entrada` LIKE ? OR `cep_direccionamiento` LIKE ? OR `cep_observacion_traslado` LIKE ? OR `cep_documento_identidad` LIKE ? OR `cep_nombre_ciudadano` LIKE ? OR `cep_correo_direccion` LIKE ? OR `cep_departamento` LIKE ? OR `cep_novedad_radicado` LIKE ? OR `cep_observaciones` LIKE ? OR `cep_notificar` LIKE ? OR `cep_registro_usuario` LIKE ? OR `cep_registro_fecha` LIKE ? OR DIRECCIONAMIENTO.`ceco_valor` LIKE ? OR DEPARTAMENTO.`ciu_departamento` LIKE ? OR NOVEDAD.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if ($permisos_usuario=="Administrador") {
      ...
      $filtro_perfil=" AND `cep_registro_usuario`=?";
      array_push($data_consulta, $_SESSION[APP_SESSION.'_session_usu_id']);
  }

  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cep_registro_fecha`>=?)";
  ...
  $consulta_string="SELECT `cep_id`, `cep_radicado_entrada`, `cep_direccionamiento`, `cep_observacion_traslado`, `cep_documento_identidad`, `cep_nombre_ciudadano`, `cep_correo_direccion`, `cep_departamento`, `cep_novedad_radicado`, `cep_observaciones`, `cep_notificar`, `cep_registro_usuario`, `cep_registro_fecha`, DIRECCIONAMIENTO.`ceco_valor`, DEPARTAMENTO.`ciu_departamento`, NOVEDAD.`ceco_valor`, TU.`usu_nombres_apellidos` FROM `gestion_cerep_proyectores` LEFT JOIN `gestion_ce_configuracion` AS DIRECCIONAMIENTO ON `gestion_cerep_proyectores`.`cep_direccionamiento`=DIRECCIONAMIENTO.`ceco_id` LEFT JOIN `administrador_departamentos` AS DEPARTAMENTO ON `gestion_cerep_proyectores`.`cep_departamento`=DEPARTAMENTO.`ciu_codigo` LEFT JOIN `gestion_ce_configuracion` AS NOVEDAD ON `gestion_cerep_proyectores`.`cep_novedad_radicado`=NOVEDAD.`ceco_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_cerep_proyectores`.`cep_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `cep_id` DESC LIMIT ?,?";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_proyectores_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_proyectores?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_proyectores?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Direccionamiento</th>
                          <th class="px-1 py-2">Observación Traslado</th>
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_proyectores_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
