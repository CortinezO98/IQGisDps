<?php
    ...
    if(isset($_POST["reporte"])){
        ...
        $titulo_reporte="Canal Escrito - Proyectores - ".date('Y-m-d H_i_s').".csv";
    ...
    fputcsv($file, array('Reporte: Canal Escrito - Proyectores'), $delimitador, $encapsulador);
    fputcsv($file, array('Fecha filtro: '.$fecha_inicio.' A '.$fecha_fin), $delimitador, $encapsulador);
    
    $titulos=array('Radicado Entrada', 'Direccionamiento', 'Observación Traslado', 'Documento Identidad', 'Nombre Ciudadano', 'Correo/Dirección Notificación', 'Departamento', 'Novedad Radicado', 'Observaciones', 'Doc Usuario Registro', 'Registrado por', 'Fecha Registro');

    fputcsv($file, $titulos, $delimitador, $encapsulador);

    for ($i=0; $i < count($resultado_registros); $i++) {
        $linea=array($resultado_registros[$i][1], $resultado_registros[$i][13], $resultado_registros[$i][3], $resultado_registros[$i][4], $resultado_registros[$i][5], $resultado_registros[$i][6], $resultado_registros[$i][14], $resultado_registros[$i][15], $resultado_registros[$i][9], $resultado_registros[$i][11], $resultado_registros[$i][16], $resultado_registros[$i][12]);
