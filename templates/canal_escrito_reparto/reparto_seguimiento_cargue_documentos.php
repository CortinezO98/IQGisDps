<?php
  ...
  $subtitle = "Reparto | 11. Seguimiento Cargue Documentos | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cescd_radicado_entrada` LIKE ? OR `cescd_radicado_salida` LIKE ? OR `cescd_novedad` LIKE ? OR `cescd_observaciones` LIKE ? OR `cescd_notificar` LIKE ? OR `cescd_registro_usuario` LIKE ? OR `cescd_registro_fecha` LIKE ? OR NOVEDAD.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if ($permisos_usuario=="Administrador") {
      ...
      $filtro_perfil=" AND `cescd_registro_usuario`=?";
      array_push($data_consulta, $_SESSION[APP_SESSION.'_session_usu_id']);
  }

  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cescd_registro_fecha`>=?)";
  ...
  $consulta_string="SELECT `cescd_id`, `cescd_radicado_entrada`, `cescd_radicado_salida`, `cescd_novedad`, `cescd_observaciones`, `cescd_notificar`, `cescd_registro_usuario`, `cescd_registro_fecha`, NOVEDAD.`ceco_valor`, TU.`usu_nombres_apellidos` FROM `gestion_cerep_seguimiento_cargue_documentos` LEFT JOIN `gestion_ce_configuracion` AS NOVEDAD ON `gestion_cerep_seguimiento_cargue_documentos`.`cescd_novedad`=NOVEDAD.`ceco_id` LEFT JOIN `administrador_usuario` AS TU ON `gestion_cerep_seguimiento_cargue_documentos`.`cescd_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `cescd_id` DESC LIMIT ?,?";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_seguimiento_cargue_documentos_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_seguimiento_cargue_documentos?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_seguimiento_cargue_documentos?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_seguimiento_cargue_documentos_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
