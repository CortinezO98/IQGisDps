<?php
    ...
    if(isset($_POST["reporte"])){
        ...
        $titulo_reporte="Canal Escrito - Seguimiento Cargue Documentos - ".date('Y-m-d H_i_s').".csv";
    ...
    fputcsv($file, array('Reporte: Canal Escrito - Seguimiento Cargue Documentos'), $delimitador, $encapsulador);
    fputcsv($file, array('Fecha filtro: '.$fecha_inicio.' A '.$fecha_fin), $delimitador, $encapsulador);
    
    $titulos=array('Radicado Entrada', 'Radicado Salida', 'Novedad', 'Observaciones', 'Doc Usuario Registro', 'Registrado por', 'Fecha Registro');

    fputcsv($file, $titulos, $delimitador, $encapsulador);

    for ($i=0; $i < count($resultado_registros); $i++) {
        $linea=array($resultado_registros[$i][1], $resultado_registros[$i][2], $resultado_registros[$i][8], $resultado_registros[$i][4], $resultado_registros[$i][6], $resultado_registros[$i][9], $resultado_registros[$i][7]);
