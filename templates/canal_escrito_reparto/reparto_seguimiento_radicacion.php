<?php
  ...
  $subtitle = "Reparto | 12. Seguimiento Radicación | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cesr_correo_ciudadano` LIKE ? OR `cesr_fecha_ingreso_correo` LIKE ? OR `cesr_dependencia` LIKE ? OR `cesr_senotifica` LIKE ? OR `cesr_observaciones` LIKE ? OR `cesr_notificar` LIKE ? OR `cesr_registro_usuario` LIKE ? OR `cesr_registro_fecha` LIKE ? OR DEPENDENCIA.`ceco_valor` LIKE ? OR NOTIFICA.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cesr_registro_fecha`>=?)";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_seguimiento_radicacion_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_seguimiento_radicacion?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_seguimiento_radicacion?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_seguimiento_radicacion_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
