<?php
  ...
  $subtitle = "Reparto | 13. Seguimiento Tipificaciones | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cest_radicado` LIKE ? OR `cest_requiere_traslado` LIKE ? OR `cest_oficio_especial` LIKE ? OR `cest_observaciones` LIKE ? OR `cest_notificar` LIKE ? OR `cest_registro_usuario` LIKE ? OR `cest_registro_fecha` LIKE ? OR TRASLADO.`ceco_valor` LIKE ? OR OFICIO.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if ($permisos_usuario=="Administrador") {
      ...
      $filtro_perfil=" AND `cest_registro_usuario`=?";
      array_push($data_consulta, $_SESSION[APP_SESSION.'_session_usu_id']);
  }

  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cest_registro_fecha`>=?)";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="reparto_seguimiento_tipificaciones_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="reparto_seguimiento_tipificaciones?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="reparto_seguimiento_tipificaciones?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Número Radicado</th>
        ...
        <!-- modal reportes -->
        <?php require_once('reparto_seguimiento_tipificaciones_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
