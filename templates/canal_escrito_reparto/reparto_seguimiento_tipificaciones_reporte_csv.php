<?php
    ...
    if(isset($_POST["reporte"])){
        ...
        $titulo_reporte="Canal Escrito - Seguimiento Tipificaciones - ".date('Y-m-d H_i_s').".csv";
    ...
    fputcsv($file, array('Reporte: Canal Escrito - Seguimiento Tipificaciones'), $delimitador, $encapsulador);
    fputcsv($file, array('Fecha filtro: '.$fecha_inicio.' A '.$fecha_fin), $delimitador, $encapsulador);
    
    $titulos=array('Número Radicado', 'Direccionamiento', 'Observaciones', 'Doc Usuario Registro', 'Registrado por', 'Fecha Registro');

    fputcsv($file, $titulos, $delimitador, $encapsulador);

    for ($i=0; $i < count($resultado_registros); $i++) {
        $linea=array($resultado_registros[$i][1], $resultado_registros[$i][9], $resultado_registros[$i][4], $resultado_registros[$i][6], $resultado_registros[$i][10], $resultado_registros[$i][7]);
