<?php
  ...
  $subtitle = "Transacciones Monetarias No Condicionadas | 7. Casos Sin Gestionar | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cetcsg_proceso_ingreso_solidario` LIKE ? OR `cetcsg_responsable_envio` LIKE ? OR `cetcsg_responsable_proyeccion` LIKE ? OR `cetcsg_causal_no_envio` LIKE ? OR `cetcsg_causal_no_proyeccion` LIKE ? OR `cetcsg_cantidad_casos` LIKE ? OR `cetcsg_observaciones` LIKE ? OR `cetcsg_notificar` LIKE ? OR `cetcsg_registro_usuario` LIKE ? OR `cetcsg_registro_fecha` LIKE ? OR INGRESOSOLIDARIO.`ceco_valor` LIKE ? OR RESPONSABLEENVIO.`ceco_valor` LIKE ? OR RESPONSABLEPROYECCION.`ceco_valor` LIKE ? OR CNOENVIO.`ceco_valor` LIKE ? OR CNPROYECCION.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cetcsg_registro_fecha`>=?)";
  ...
  $consulta_contar_string="SELECT COUNT(`cetcsg_id`) FROM `gestion_cetmnc_casos_sin_gestionar` LEFT JOIN `gestion_ce_configuracion` AS INGRESOSOLIDARIO ON `gestion_cetmnc_casos_sin_gestionar`.`cetcsg_proceso_ingreso_solidario`=INGRESOSOLIDARIO.`ceco_id`
   ...
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cetmnc_casos_sin_gestionar`.`cetcsg_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  $consulta_string="SELECT `cetcsg_id`, `cetcsg_proceso_ingreso_solidario`, `cetcsg_responsable_envio`, `cetcsg_responsable_proyeccion`, `cetcsg_causal_no_envio`, `cetcsg_causal_no_proyeccion`, `cetcsg_cantidad_casos`, `cetcsg_observaciones`, `cetcsg_notificar`, `cetcsg_registro_usuario`, `cetcsg_registro_fecha`, INGRESOSOLIDARIO.`ceco_valor`, RESPONSABLEENVIO.`ceco_valor`, RESPONSABLEPROYECCION.`ceco_valor`, CNOENVIO.`ceco_valor`, CNPROYECCION.`ceco_valor`, TU.`usu_nombres_apellidos` FROM `gestion_cetmnc_casos_sin_gestionar`
   ...
   LEFT JOIN `gestion_ce_configuracion` AS CNOENVIO ON `gestion_cetmnc_casos_sin_gestionar`.`cetcsg_causal_no_envio`=CNOENVIO.`ceco_id`
   LEFT JOIN `gestion_ce_configuracion` AS CNPROYECCION ON `gestion_cetmnc_casos_sin_gestionar`.`cetcsg_causal_no_proyeccion`=CNPROYECCION.`ceco_id`
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cetmnc_casos_sin_gestionar`.`cetcsg_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `cetcsg_id` DESC LIMIT ?,?";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="tmnc_scasos_sgestionar_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="tmnc_scasos_sgestionar?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="tmnc_scasos_sgestionar?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Proceso ingreso solidario</th>
                          <th class="px-1 py-2">Responsable envíos</th>
                          <th class="px-1 py-2">Responsable proyección</th>
                          <th class="px-1 py-2">Causal no envío</th>
                          <th class="px-1 py-2">Causal no proyección</th>
                          <th class="px-1 py-2">Cantidad casos pendientes por gestionar y/o escalados</th>
        ...
        <!-- modal reportes -->
        <?php require_once('tmnc_scasos_sgestionar_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
