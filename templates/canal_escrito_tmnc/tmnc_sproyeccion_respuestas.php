<?php
  ...
  $subtitle = "Transacciones Monetarias No Condicionadas | 1. Proyección de Respuestas | ".$bandeja;
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`cet_radicado_entrada` LIKE ? OR `cet_abogado_aprobacion` LIKE ? OR `cet_documento_identidad` LIKE ? OR `cet_nombre_ciudadano` LIKE ? OR `cet_correo_direccion` LIKE ? OR `cet_programa_solicitud` LIKE ? OR `cet_plantilla` LIKE ? OR `cet_con_datos` LIKE ? OR `cet_datos_incompletos` LIKE ? OR `cet_plantilla_compensacion_iva` LIKE ? OR `cet_plantilla_adulto_mayor` LIKE ? OR `cet_novedad_radicado` LIKE ? OR `cet_motivo_archivo` LIKE ? OR `cet_tipo_entidad` LIKE ? OR `cet_id_solicitud` LIKE ? OR `cet_observaciones` LIKE ? OR `cet_notificar` LIKE ? OR `cet_registro_usuario` LIKE ? OR `cet_registro_fecha` LIKE ? OR ABOGADOAPROBACION.`ceco_valor` LIKE ? OR PROGRAMASOLICITUD.`ceco_valor` LIKE ? OR PLANTILLA.`ceco_valor` LIKE ? OR CONDATOS.`ceco_valor` LIKE ? OR DATOSINCOMPLETOS.`ceco_valor` LIKE ? OR PCOMPENSACIONIVA.`ceco_valor` LIKE ? OR PADULTOMAYOR.`ceco_valor` LIKE ? OR NOVEDADRADICADO.`ceco_valor` LIKE ? OR TIPOENTIDAD.`ceco_valor` LIKE ? OR TU.`usu_nombres_apellidos` LIKE ?)";
  ...
  if ($permisos_usuario=="Administrador") {
      ...
      $filtro_perfil=" AND `cet_registro_usuario`=?";
      array_push($data_consulta, $_SESSION[APP_SESSION.'_session_usu_id']);
  }

  if($bandeja=="Hoy"){
      $filtro_bandeja=" AND (`cet_registro_fecha`>=?)";
  ...
  $consulta_contar_string="SELECT COUNT(`cet_id`) FROM `gestion_cetmnc_proyeccion_respuestas` LEFT JOIN `gestion_ce_configuracion` AS ABOGADOAPROBACION ON `gestion_cetmnc_proyeccion_respuestas`.`cet_abogado_aprobacion`=ABOGADOAPROBACION.`ceco_id`
 ...
 LEFT JOIN `administrador_usuario` AS TU ON `gestion_cetmnc_proyeccion_respuestas`.`cet_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja."";
  ...
  $consulta_string="SELECT `cet_id`, `cet_radicado_entrada`, `cet_abogado_aprobacion`, `cet_documento_identidad`, `cet_nombre_ciudadano`, `cet_correo_direccion`, `cet_programa_solicitud`, `cet_plantilla`, `cet_con_datos`, `cet_datos_incompletos`, `cet_plantilla_compensacion_iva`, `cet_plantilla_adulto_mayor`, `cet_novedad_radicado`, `cet_motivo_archivo`, `cet_tipo_entidad`, `cet_id_solicitud`, `cet_observaciones`, `cet_notificar`, `cet_registro_usuario`, `cet_registro_fecha`, ABOGADOAPROBACION.`ceco_valor`, PROGRAMASOLICITUD.`ceco_valor`, PLANTILLA.`ceco_valor`, CONDATOS.`ceco_valor`, DATOSINCOMPLETOS.`ceco_valor`, PCOMPENSACIONIVA.`ceco_valor`, PADULTOMAYOR.`ceco_valor`, NOVEDADRADICADO.`ceco_valor`, TIPOENTIDAD.`ceco_valor`, TU.`usu_nombres_apellidos`, PRENTA.`ceco_valor` FROM `gestion_cetmnc_proyeccion_respuestas`
   ...
   LEFT JOIN `gestion_ce_configuracion` AS NOVEDADRADICADO ON `gestion_cetmnc_proyeccion_respuestas`.`cet_novedad_radicado`=NOVEDADRADICADO.`ceco_id`
   LEFT JOIN `gestion_ce_configuracion` AS TIPOENTIDAD ON `gestion_cetmnc_proyeccion_respuestas`.`cet_tipo_entidad`=TIPOENTIDAD.`ceco_id`
   LEFT JOIN `administrador_usuario` AS TU ON `gestion_cetmnc_proyeccion_respuestas`.`cet_registro_usuario`=TU.`usu_id` WHERE 1=1 ".$filtro_buscar." ".$filtro_perfil." ".$filtro_bandeja." ORDER BY `cet_id` DESC LIMIT ?,?";
  ...
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      <div class="main-panel">
            ...
            <div class="col-md-9">
              <div class="row">
                ...
                <div class="col-md-7 mb-1 text-end">
                  <a href="tmnc_sproyeccion_respuestas_crear?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode($bandeja); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                    <i class="fas fa-plus btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Crear Registro</span>
                  </a>
                  <a href="tmnc_sproyeccion_respuestas?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Hoy'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Hoy">
                    <i class="fas fa-clock btn-icon-prepend me-0 me-lg-1 font-size-12"></i><span class="d-none d-lg-inline">Hoy</span>
                  </a>
                  <a href="tmnc_sproyeccion_respuestas?pagina=1&id=<?php echo $filtro_permanente; ?>&bandeja=<?php echo base64_encode('Histórico'); ?>" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Histórico">
                    ...
                    <table class="table table-hover table-bordered table-striped">
                      <thead>
                        <tr>
                          ...
                          <th class="px-1 py-2">Abogado asignado para aprobación</th>
                          <th class="px-1 py-2">Documento identidad</th>
                          <th class="px-1 py-2">Nombre del ciudadano</th>
                          <th class="px-1 py-2">Correo/dirección de notificación</th>
                          ...
                          <th class="px-1 py-2">Plantilla compensación IVA</th>
                          <th class="px-1 py-2">Plantilla adulto mayor</th>
                          <th class="px-1 py-2">Plantilla renta ciudadana</th>
                          <th class="px-1 py-2">Novedad radicado</th>
                          <th class="px-1 py-2">Motivo porque se archiva</th>
                          <th class="px-1 py-2">Tipo de entidad</th>
                          <th class="px-1 py-2">Id solicitud</th>
        ...
        <!-- modal reportes -->
        <?php require_once('tmnc_sproyeccion_respuestas_reporte.php'); ?>
      </div>
      <!-- main-panel -->
    </div>
  </div>
