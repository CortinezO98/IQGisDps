<?php
/**
 * templates/gas_publico/encuesta_procesar.php — CORREGIDO
...
$stmt = $enlace_db->prepare(
    "SELECT gas_id, gas_nombre, gas_facilitador
     ...
     WHERE gas_token_publico = ? AND gas_estado = 'finalizada' LIMIT 1"
);
...
if (!$asistencia) {
    $error_tipo = 'no_registrado'; include __DIR__ . '/error.php'; exit;
}
...
// 12 parámetros: ii(sesion+asistencia) + iiiii(5 cal) + s(obs) + d(prom) + sss(fecha,ip,ua)
$stmt = $enlace_db->prepare(
...
if ($stmt->execute()) {
    ...
    include __DIR__ . '/encuesta_confirmacion.php'; exit;
} else {
