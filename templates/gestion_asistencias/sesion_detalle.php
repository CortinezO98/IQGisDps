 ...
 * Detalle completo de una sesión: info, QR, link, asistentes, encuestas, cambio de estado.
 */
...
$stmt->bind_param('i', $id);
$stmt->execute();
$ultimos_asistentes = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();
    ...
    <div class="container-fluid">
      ...
      <?php if ($nuevo): ?>
        <div class="alert alert-success alert-dismissible fade show">
          ...
          <strong>¡Sesión creada exitosamente!</strong>
          El código de sesión es <strong><?= htmlspecialchars($sesion['gas_codigo']) ?></strong>.
          Comparte el link o QR con los asistentes cuando la actives.
          ...
          <div class="page-title-box d-flex align-items-center justify-content-between">
            <div>
              ...
              <small class="text-muted">
                ...
                &nbsp;·&nbsp;
                <?= htmlspecialchars(GAS_TIPOS_SESION[$sesion['gas_tipo_sesion']] ?? $sesion['gas_tipo_sesion']) ?>
                &nbsp;·&nbsp;
      ...
      <div class="row g-3">
        ...
        <div class="col-md-8">
          <div class="card h-100">
            <div class="card-header"><h6 class="mb-0">Información de la Sesión</h6></div>
            <div class="card-body">
              <dl class="row mb-0">
                <dt class="col-sm-4">Facilitador</dt>
              ...
              <div class="input-group mb-2">
                <input type="text" id="link_publico" class="form-control form-control-sm"
        ...
        <div class="col-md-4">
          ...
          <div class="card mb-3 text-center">
            <div class="card-header"><h6 class="mb-0">Código QR</h6></div>
            <div class="card-body">
              <?php if (!empty($sesion['gas_qr_path']) && file_exists(__DIR__ . '/../../../' . $sesion['gas_qr_path'])): ?>
                <img src="<?= GAS_BASE_URL . '/' . htmlspecialchars($sesion['gas_qr_path']) ?>"
                     alt="QR Sesión" class="img-fluid mb-2" style="max-width:180px;">
                <br>
                <a href="<?= GAS_BASE_URL . '/' . htmlspecialchars($sesion['gas_qr_path']) ?>"
              ...
              <?php else: ?>
                <div class="text-muted py-3">
                  ...
                  <p class="small">QR no disponible.<br>Instala phpqrcode para generarlo.</p>
                </div>
              <?php endif; ?>
            </div>
          </div>
          ...
          <!-- Métricas -->
          <div class="card">
            <div class="card-header"><h6 class="mb-0">Métricas</h6></div>
            <div class="card-body">
              <div class="row text-center g-2">
                ...
                <?php endif; ?>
                <?php if ($metricas['prom_general'] !== null): ?>
                <div class="col-12 mt-2">
                  <table class="table table-sm table-borderless mb-0 text-start">
                    <tr><td><small>Promedio General</small></td><td class="text-end fw-bold"><?= number_format((float)$metricas['prom_general'], 1) ?>/5</td></tr>
                    <tr><td><small>Facilitador</small></td><td class="text-end"><?= number_format((float)$metricas['prom_facilitador'], 1) ?>/5</td></tr>
                    <tr><td><small>Metodología</small></td><td class="text-end"><?= number_format((float)$metricas['prom_metodologia'], 1) ?>/5</td></tr>
                    <tr><td><small>Material</small></td><td class="text-end"><?= number_format((float)$metricas['prom_material'], 1) ?>/5</td></tr>
                  </table>
                </div>
                <?php endif; ?>
              </div>
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h6 class="mb-0">Últimos Asistentes Registrados</h6>
            ...
            <div class="card-body p-0">
              <div class="table-responsive">
                <table class="table table-sm align-middle mb-0">
                  ...
                  <tbody>
                    <?php foreach ($ultimos_asistentes as $a): ?>
                      <tr>
...
<!-- Modal cambio de estado (oculto) -->
<form id="form_estado" method="POST" action="sesion_cambiar_estado.php" style="display:none;">
  <input type="hidden" name="sesion_id" id="input_sesion_id">
  <input type="hidden" name="nuevo_estado" id="input_nuevo_estado">
...
function cambiarEstado(sesionId, nuevoEstado, accion) {
    if (!confirm('¿Confirmas que deseas ' + accion.toLowerCase() + ' esta sesión?')) return;
    document.getElementById('input_sesion_id').value   = sesionId;
    document.getElementById('input_nuevo_estado').value = nuevoEstado;
