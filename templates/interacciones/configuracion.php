<?php
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`gia_campo` LIKE ? OR `gia_tipo` LIKE ? OR `gia_nombre` LIKE ? OR `gia_estado` LIKE ? OR `gia_opciones` LIKE ?)";
  ...
  $consulta_contar_string="SELECT COUNT(`gia_id`) FROM `gestion_interacciones_auxiliar` WHERE 1=1 ".$filtro_buscar."";
  ...
  $consulta_string="SELECT `gia_id`, `gia_campo`, `gia_tipo`, `gia_nombre`, `gia_estado`, `gia_opciones` FROM `gestion_interacciones_auxiliar` WHERE 1=1 ".$filtro_buscar." ORDER BY `gia_id` ASC LIMIT ?,?";
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="p-1 text-center">
                          <a href="configuracion_editar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-warning btn-icon px-1 py-1 mb-1" title="Editar"><i class="fas fa-pen font-size-11"></i></a>
                      </td>
