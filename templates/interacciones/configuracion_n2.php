<?php
  ...
  /*VARIABLES*/
  $title = "Interacciones";
  $subtitle = "Configuración | Tipificación 2";
  ...
  if ($filtro_permanente!="null" AND $filtro_permanente!="") {
      $filtro_buscar="AND (`gic2_item` LIKE ? OR `gic2_estado` LIKE ? OR TP.`gic1_item` LIKE ?)";
  ...
  $consulta_contar_string="SELECT COUNT(`gic2_id`) FROM `gestion_interacciones_catnivel2` LEFT JOIN `gestion_interacciones_catnivel1` AS TP ON `gestion_interacciones_catnivel2`.`gic2_padre`=TP.`gic1_id` WHERE 1=1 AND `gic2_id`>0 ".$filtro_buscar."";
  ...
  $consulta_string="SELECT `gic2_id`, `gic2_padre`, `gic2_item`, `gic2_estado`, `gic2_registro_usuario`, `gic2_registro_fecha`, TP.`gic1_item`, TP.`gic1_estado` FROM `gestion_interacciones_catnivel2` LEFT JOIN `gestion_interacciones_catnivel1` AS TP ON `gestion_interacciones_catnivel2`.`gic2_padre`=TP.`gic1_id` WHERE 1=1 AND `gic2_id`>0 ".$filtro_buscar." ORDER BY TP.`gic1_item`, `gic2_item` LIMIT ?,?";
        ...
        <div class="content-wrapper">
          <div class="row">
            ...
            <div class="col-md-7 mb-1 text-end">
              ...
              <a href="configuracion_n2_crear?pagina=1&id=null" class="btn py-2 px-2 btn-primary btn-corp btn-icon-text font-size-12" title="Crear Registro">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="p-1 text-center">
                        <?php if($resultado_registros[$i][7]=='Activo'): ?>
                          <a href="configuracion_n2_editar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-warning btn-icon px-1 py-1 mb-1" title="Editar"><i class="fas fa-pen font-size-11"></i></a>
                          <a href="configuracion_n2_eliminar?pagina=<?php echo $pagina; ?>&id=<?php echo $filtro_permanente; ?>&reg=<?php echo base64_encode($resultado_registros[$i][0]); ?>" class="btn btn-danger btn-icon px-1 py-1 mb-1" title="Eliminar"><i class="fas fa-trash-alt font-size-11"></i></a>
                        <?php endif; ?>
                      </td>
