<?php
	...
	$mails_folder=$mail->get_mails_folder($guzzle, $array_folders['iqgis_radicados_pendiente']['id']);
	...
	// FILTRO comprobantes de entrega
    $array_notificaciones_correo = array(
        "No se puede entregar:  Gestión de la petición",
        "No se puede entregar: Gestióndela petición",
        "No se puede entregar: Traslado por competencia",
        "No se puede entregar: Trasladopor competencia"
    );
	...
	if ($limite_lectura>0) {
		...
		for ($i=0; $i < $limite_lectura; $i++) { 
			...
			if ($mensaje_de!="" AND $mensaje_de!="@" AND $mensaje_asunto!="") {
		        ...
		        $consulta_registro_correo_ingreso_insert = $enlace_db->prepare("INSERT INTO `gestion_radicacion_casos`(`grc_radicado`, `grc_tipologia`, `grc_clasificacion`, `grc_responsable`, `grc_gestion`, `grc_gestion_detalle`, `grc_estado`, `grc_duplicado`, `grc_unificado`, `grc_unificado_id`, `grc_dividido`, `grc_dividido_cantidad`, `grc_fecha_gestion`, `grc_correo_remitente`, `grc_correo_asunto`, `grc_correo_fecha`, `grc_correo_fecha_amd`, `grc_correo_fecha_anio`, `grc_correo_fecha_mes`, `grc_correo_fecha_dia`, `grc_registro_fecha`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");

		        // Agrega variables a sentencia preparada
		        $consulta_registro_correo_ingreso_insert->bind_param('sssssssssssssssssssss', $grc_radicado, $grc_tipologia, $grc_clasificacion, $grc_responsable, $grc_gestion, $grc_gestion_detalle, $grc_estado, $grc_duplicado, $grc_unificado, $grc_unificado_id, $grc_dividido, $grc_dividido_cantidad, $grc_fecha_gestion, $grc_correo_remitente, $grc_correo_asunto, $grc_correo_fecha, $grc_correo_fecha_amd, $grc_correo_fecha_anio, $grc_correo_fecha_mes, $grc_correo_fecha_dia, $grc_registro_fecha);
		        ...
		        //Tipificación
			        //$array_notificaciones_correo
		        	// Comprobar si el asunto contiene alguna de las palabras clave
		            $filtered = false;
		            foreach ($array_notificaciones_correo as $keyword) {
		                if (stripos($subject, $keyword) === 0) {
		                    $grc_tipologia = 'Notificaciones de correo';
					    	$grc_clasificacion = 'No se puede entregar';
		                    $tipificado=true;
		                    break;
		            	}
					}
				...
				if ($grc_tipologia=='Notificaciones de correo' AND $grc_clasificacion=='') {
				...
				$grc_correo_fecha_amd=date('Y-m-d', strtotime($fecha_enviado_registro));
				$grc_correo_fecha_anio=date('Y', strtotime($fecha_enviado_registro));
				$grc_correo_fecha_mes=date('m', strtotime($fecha_enviado_registro));
				$grc_correo_fecha_dia=date('d', strtotime($fecha_enviado_registro));
				$grc_registro_fecha=date('Y-m-d');
